...
function progressKey(value: string) {
  let hash = 2166136261;
  for (let index = 0; index < value.length; index++) {
    hash ^= value.charCodeAt(index);
    hash = Math.imul(hash, 16777619);
  }
  return `test-builder-progress-${(hash >>> 0).toString(36)}`;
}
...
async function createTestHtml() {
  ...
  const storageKey = progressKey(projectData);
...
</style></head><body><main class="wrap"><header><h1 id="title"></h1><p>Answer each question, then submit to see your score.</p></header><form id="quiz"></form><section class="results" id="results"><p>Score</p><div class="score" id="score"></div><div class="review" id="review"></div><button id="retry" type="button">Restart test</button></section></main>
<script>window.addEventListener('DOMContentLoaded',()=>{const key=${JSON.stringify(storageKey)};const quiz=document.getElementById('quiz');const retry=document.getElementById('retry');let state={answers:{},submitted:false};try{state=JSON.parse(localStorage.getItem(key))||state}catch{}Object.entries(state.answers||{}).forEach(([name,value])=>{const fields=[...document.querySelectorAll('[name="'+CSS.escape(name)+'"]')];if(Array.isArray(value)){fields.forEach(field=>field.checked=value.includes(field.value))}else if(fields[0]){if(fields[0].type==='radio'||fields[0].type==='checkbox')fields.forEach(field=>field.checked=field.value===value);else fields[0].value=value}});const collect=()=>{const answers={};document.querySelectorAll('[name]').forEach(field=>{if(field.type==='checkbox'){answers[field.name]??=[];if(field.checked)answers[field.name].push(field.value)}else if(field.type==='radio'){if(field.checked)answers[field.name]=field.value}else answers[field.name]=field.value});return answers};const save=submitted=>{state={answers:collect(),submitted};try{localStorage.setItem(key,JSON.stringify(state))}catch{}};quiz.addEventListener('input',()=>save(false));quiz.addEventListener('change',()=>save(false));quiz.addEventListener('submit',()=>save(true),true);retry.addEventListener('click',()=>{try{localStorage.removeItem(key)}catch{}});if(state.submitted)setTimeout(()=>quiz.requestSubmit(),0)});</script>
